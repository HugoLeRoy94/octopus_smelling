...
in \cite{MWC}, they show that this function is over-parametrized, meaning that several combination of values for $K_c$, $K_o$ and $\epsilon$ can lead to the same curve. As a result, they find a simplified model by setting : $\tilde{K}_o = K_o e^{\beta \epsilon }$:
$$
...
\begin{figure*}[t]
    \includegraphics[width=0.9\textwidth]{MI_sensors.pdf}
    \caption{\label{fig:3}}
\end{figure*}
